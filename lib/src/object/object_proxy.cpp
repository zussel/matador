...
basic_object_proxy::basic_object_proxy() {}

basic_object_proxy::basic_object_proxy(unsigned long id, object_store *store)
  : oid(id)
  , ostore_(store)
{}

basic_object_proxy::basic_object_proxy(const std::shared_ptr<basic_identifier> &pk)
  : primary_key_(pk)
{}

object_store *basic_object_proxy::ostore() const
{
  return ostore_;
}

prototype_node *basic_object_proxy::node() const
{
  return node_;
}

void basic_object_proxy::link(basic_object_proxy *successor)
{
...
void basic_object_proxy::unlink()
{
  if (prev_) {
    prev_->next_ = next_;
  }
...
void basic_object_proxy::link_ref()
{
  if (obj_) {
    ++ref_count_;
  }
...
void basic_object_proxy::unlink_ref()
{
  if (obj_) {
    --ref_count_;
  }
...
void basic_object_proxy::link_ptr()
{
  if (obj_) {
    ++ptr_count_;
  }
...
void basic_object_proxy::unlink_ptr()
{
  if (obj_) {
    --ptr_count_;
  }
...
bool basic_object_proxy::linked() const
{
  return node_ != 0;
}

basic_object_proxy *basic_object_proxy::next() const
{
  return next_;
}

basic_object_proxy *basic_object_proxy::prev() const
{
  return prev_;
}

unsigned long basic_object_proxy::ref_count() const
{
  return ref_count_;
}

unsigned long basic_object_proxy::ptr_count() const
{
  return ptr_count_;
}
void basic_object_proxy::add(object_base_ptr *ptr)
{
  ptr_set_.insert(ptr);
}

bool basic_object_proxy::remove(object_base_ptr *ptr)
{
  return ptr_set_.erase(ptr) == 1;
}

bool basic_object_proxy::valid() const
{
  return ostore_ && node_ && prev_ && next_;
}

unsigned long basic_object_proxy::id() const
{
  return oid;
}

void basic_object_proxy::id(unsigned long i)
{
  oid = i;
}

bool basic_object_proxy::has_primary_key() const
{
  return primary_key_ != nullptr;
}

std::shared_ptr<basic_identifier> basic_object_proxy::pk() const
{
  return primary_key_;
}
