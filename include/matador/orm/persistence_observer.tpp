...
template<class T>
void persistence_observer<matador::has_many_item<T>>::on_attach(prototype_node &node, matador::has_many_item<T> &)
{
  if (persistence_.tables_.find(node.type()) != persistence_.tables_.end()) {
    return;
  }
...
template<class T>
void persistence_observer<matador::has_many_item<T>>::on_detach(prototype_node &node, matador::has_many_item<T> &)
{
  auto i = persistence_.tables_.find(node.type());
  if (i == persistence_.tables_.end()) {
    return;
