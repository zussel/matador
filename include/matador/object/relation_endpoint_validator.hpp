...
public:
  ...
  void on_has_many(const char * id, container<Value, Container> &, const char *, const foreign_attributes &/*attr*/ = default_foreign_attributes )
  {
  ...
  template<class Value, template<class ...> class Container>
  void on_has_many(const char * id, container<Value, Container> &, const foreign_attributes &/*attr*/ = default_foreign_attributes )
  {
    if (!is_valid_ && relation_type_ == basic_relation_endpoint::HAS_MANY) {
      is_valid_ = column_name_ == id;
    }
  ...
  template<class Value, template<class ...> class Container>
  void on_has_many_to_many(const char *, container<Value, Container> &, const char *, const char *, const foreign_attributes &/*attr*/ = default_foreign_attributes) {}
