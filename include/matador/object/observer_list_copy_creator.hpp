...
{
public:
  using source_observer_vector = std::vector<std::unique_ptr<typed_object_store_observer<SourceType>>>;
  using observer_vector = std::vector<std::unique_ptr<typed_object_store_observer<DestType>>>;
  ...
  static observer_vector copy_create(const source_observer_vector &source_observers) {
    observer_list_copy_creator<SourceType, DestType, ObserverType...> creator(source_observers);
