...
/**
 * @class generic_json_parser_cstring
...
protected:
  /**
   * Creates a new generic_json_parser_cstring for
   * concrete parser h.
   *
   * @param h The concrete parser.
   */
  explicit generic_json_parser_cstring(T *h) : handler_(h) {}
...
public:
  virtual ~generic_json_parser_cstring() = default;
...
template < class T > const char *generic_json_parser_cstring<T>::null_string = "null";
template < class T > const char *generic_json_parser_cstring<T>::true_string = "true";
template < class T > const char *generic_json_parser_cstring<T>::false_string = "false";

template < class T >
void
generic_json_parser_cstring<T>::parse_json(std::istream &in)
{
  // eat whitespace
  in >> std::ws;
...
template < class T >
void
generic_json_parser_cstring<T>::parse_json_object(std::istream &in)
{
  in >> std::ws;
...
template < class T >
void
generic_json_parser_cstring<T>::parse_json_array(std::istream &in)
{
  // skip white
  in >> std::ws;
...
template < class T >
std::string
generic_json_parser_cstring<T>::parse_json_string(std::istream &in)
{
  // skip white
  in >> std::ws;
...
template < class T >
typename generic_json_parser_cstring<T>::real_t
generic_json_parser_cstring<T>::parse_json_number(std::istream &in)
{
  // skip white
  in >> std::ws;
...
template < class T >
bool
generic_json_parser_cstring<T>::parse_json_bool(std::istream &in)
{
  // skip white
  in >> std::ws;
  ...
  if (c == 't') {
    ...
    while (in && i < 4) {
      ...
      if (generic_json_parser_cstring<T>::true_string[i++] != c) {
        // TODO: throw json_error instead
    ...
    while (in && i < 5) {
      ...
      if (generic_json_parser_cstring<T>::false_string[i++] != c) {
        // TODO: throw json_error instead
        throw std::logic_error("invalid bool character");
      }
    }
...
template < class T >
void
generic_json_parser_cstring<T>::parse_json_null(std::istream &in)
{
  // skip white
  in >> std::ws;
  ...
  if (c == 'n') {
    ...
    while (in && i < 4) {
      ...
      if (generic_json_parser_cstring<T>::null_string[i++] != c) {
        // TODO: throw json_error instead
        throw std::logic_error("invalid bool character");
      }
    }
...
template < class T >
void
generic_json_parser_cstring<T>::parse_json_value(std::istream &in)
{
  // skip ws
  in >> std::ws;
