...
query_builder &query_builder::create()
{
  ...
  query_parts_.emplace_back(dialect_token::CREATE, dialect_.token_at(dialect_token::CREATE));
...
query_builder &query_builder::drop()
{
  ...
  query_parts_.emplace_back(dialect_token::DROP, dialect_.token_at(dialect_token::DROP));
...
query_builder &query_builder::select(const std::vector<column> &columns)
{
  ...
  query_parts_.emplace_back(dialect_token::SELECT, dialect_.token_at(dialect_token::SELECT) + " ");
  ...
  query_parts_.emplace_back(dialect_token::COLUMNS, result);
  return *this;
}
...
query_builder &query_builder::insert()
{
  ...
  query_parts_.emplace_back(dialect_token::INSERT, dialect_.token_at(dialect_token::INSERT));
...
query_builder &query_builder::update(const std::string &table)
{
  ...
  query_.table = {table};
  query_parts_.emplace_back(dialect_token::UPDATE, dialect_.token_at(dialect_token::UPDATE) + " " + dialect_.prepare_identifier_string(table));
...
query_builder &query_builder::remove()
{
  ...
  query_parts_.emplace_back(dialect_token::REMOVE, dialect_.token_at(dialect_token::REMOVE));

  return *this;
}

query_builder &query_builder::table(const std::string &table, const std::initializer_list<column_definition> columns)
{
...
query_builder &query_builder::table(const std::string &table, const std::vector<column_definition> &/*columns*/)
{
  ...
  query_parts_.emplace_back(dialect_token::TABLE, " " + dialect_.token_at(dialect_token::TABLE) + " " + dialect_.prepare_identifier_string(table) + " ");
  query_.table = {table};
...
query_builder &query_builder::into(const std::string &table, const std::vector<column> &column_names)
{
  ...
  query_parts_.emplace_back(dialect_token::INTO, " " + dialect_.token_at(dialect_token::INTO) + " " + dialect_.prepare_identifier_string(table) + " ");
  query_.table = {table};
...
query_builder &query_builder::values(const std::vector<utils::any_type> &values)
{
  ...
  query_parts_.emplace_back(dialect_token::VALUES, " " + dialect_.token_at(dialect_token::VALUES) + " ");
  ...
  query_parts_.emplace_back(dialect_token::INSERT_VALUES, result);
  return *this;
}
...
query_builder &query_builder::from(const std::string &table, const std::string &as)
{
  ...
  if (dialect_.default_schema_name().empty()) {
    query_parts_.emplace_back(dialect_token::FROM, " " + dialect_.token_at(dialect_token::FROM) +
                                                      " " + dialect_.prepare_identifier_string(table) +
                                                      (as.empty() ? "" : " AS " + dialect_.prepare_identifier_string(as)));
  } else {
    query_parts_.emplace_back(dialect_token::FROM, " " + dialect_.token_at(dialect_token::FROM) +
                                                      " " + dialect_.prepare_identifier_string(dialect_.default_schema_name()) +
                                                      "." + dialect_.prepare_identifier_string(table) +
                                                      (as.empty() ? "" : " AS " + dialect_.prepare_identifier_string(as)));
  }
  query_.table = {table};
...
query_builder &query_builder::join(const std::string &table, join_type_t, const std::string &as)
{
  ...
  query_parts_.emplace_back(dialect_token::JOIN, " " + dialect_.token_at(dialect_token::JOIN) +
                                                    " " + dialect_.prepare_identifier_string(table) +
                                                    (as.empty() ? "" : " AS " + dialect_.prepare_identifier_string(as)));
...
query_builder &query_builder::on(const std::string &column, const std::string &join_column)
{
  ...
  query_parts_.emplace_back(dialect_token::ON, " " + dialect_.token_at(dialect_token::ON) +
                                                  " " + dialect_.prepare_identifier_string(column) +
                                                  "=" + dialect_.prepare_identifier_string(join_column));
...
query_builder &query_builder::set(const std::vector<key_value_pair> &key_values)
{
  ...
  query_parts_.emplace_back(dialect_token::SET, " " + dialect_.token_at(dialect_token::SET) + " ");
  ...
  query_parts_.emplace_back(dialect_token::UPDATE_VALUES, result);
  return *this;
}
...
query_builder &query_builder::where(const basic_condition &cond)
{
  ...
  query_parts_.emplace_back(dialect_token::WHERE, " " + dialect_.token_at(dialect_token::WHERE) + " ");
  query_parts_.emplace_back(dialect_token::WHERE_CLAUSE, cond.evaluate(const_cast<dialect &>(dialect_), query_));
...
query_builder &query_builder::order_by(const std::string &column)
{
  ...
  query_parts_.emplace_back(dialect_token::ORDER_BY, " " + dialect_.token_at(dialect_token::ORDER_BY) + " " + dialect_.prepare_identifier_string(column));
...
query_builder &query_builder::group_by(const std::string &column)
{
  ...
  query_parts_.emplace_back(dialect_token::GROUP_BY, " " + dialect_.token_at(dialect_token::GROUP_BY) + " " + dialect_.prepare_identifier_string(column));
...
query_builder &query_builder::asc()
{
  ...
  query_parts_.emplace_back(dialect_token::ASC, " " + dialect_.token_at(dialect_token::ASC));
...
query_builder &query_builder::desc()
{
  ...
  query_parts_.emplace_back(dialect_token::DESC, " " + dialect_.token_at(dialect_token::DESC));
...
query_builder &query_builder::offset(size_t count)
{
  ...
  query_parts_.emplace_back(dialect_token::OFFSET, " " + dialect_.token_at(dialect_token::OFFSET) + " " + std::to_string(count));
...
query_builder &query_builder::limit(size_t count)
{
  ...
  query_parts_.emplace_back(dialect_token::LIMIT, " " + dialect_.token_at(dialect_token::LIMIT) + " " + std::to_string(count));
