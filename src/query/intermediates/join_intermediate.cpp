...
on_intermediate join_intermediate::on(std::unique_ptr<abstract_criteria> &&cond) {
  return on_clause(std::move(cond));
}

on_intermediate join_intermediate::on_clause(std::unique_ptr<abstract_criteria> &&cond) {
