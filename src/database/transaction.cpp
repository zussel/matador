...
//  cout << "inserting " << *o << endl;
  ...
  if (i == id_map_.end()) {
    ...
    insert_action *a = new insert_action;
    a->push_back(o);
    action_list_.push_back(a);
  } else {
    insert_action::iterator j = (*i)->find(o->id());
    if (j == (*i)->end()) {
      (*i)->push_back(o);
    } else {
...
transaction::backup(action *a)
{
  ...
  id_map_.insert(std::make_pair(a->obj()->id(), i));
}
